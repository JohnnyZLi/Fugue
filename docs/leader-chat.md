...
- Fake manifests are processed through bounded status-ID recovery slices. Progress is persisted as write-once OIDC-signed repository Actions Variables created by the dedicated Fugue Authority GitHub App, not in issue comments or `refs/fugue/**`. Candidate `GITHUB_TOKEN` permissions do not include repository Variables. Compaction keeps one deterministic greatest cursor for every active d3 scope and never evicts another active scope's sole cursor; only quiescent completed cursors are globally reclaimable under finite-capacity pressure, so concurrent compactors and more than 24 simultaneous recoveries retain monotonic progress without Human repository surgery.
