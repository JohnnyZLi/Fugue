...
- Fake manifests are processed through bounded status-ID recovery slices. Progress is persisted as write-once OIDC-signed repository Actions Variables created by the dedicated Fugue Authority GitHub App, not in issue comments or `refs/fugue/**`. Candidate `GITHUB_TOKEN` permissions do not include repository Variables. Reverse-chronological API pages are never durable cursor coordinates: every slice re-seeks the signed `before_id` by status ID, so arbitrarily many newer statuses between slices cannot starve the frozen range. The recovery plane never evicts another resumable scope’s sole cursor merely because that scope is caught up. Greatest signed cursors are packed into immutable content-addressed Authority-variable buckets; replacement packs become durable before source cleanup. Reserved empty Authority slots provide normal hard-cap headroom, and depleted reserves trigger repository-wide safe dedup/packing before a required cursor allocation can fail, preserving all sole greatest cursors and unrelated variables. Concurrent compactors therefore preserve monotonic low-water/current-authority progress without Human repository surgery.
