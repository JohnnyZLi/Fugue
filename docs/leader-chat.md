...
- Status-only/fake manifests never become authority. After the manifest receives its exact server ID, protected Fugue must still prove the expected protected default-branch revision is current and re-check it immediately before the Authority App makes the OIDC-signed create-only witness durable. The witness binds the exact manifest ID, exact chunk IDs, second manifest proof, and current greatest authority body. Readers validate that witness directly instead of walking live reverse-chronological status pages, so hostile appends cannot move a seek/scan frontier during recovery. For a scope, greatest authority is selected by the protocol's signed logical authority order rather than manifest ID, variable name, hash, or completion timing. Greatest signed witnesses remain packed into immutable content-addressed Authority buckets. Hard-cap compaction atomically transforms an occupied verified source slot into its replacement pack, can atomically transfer a redundant source/reserve slot to a required checkpoint, and can replace a partial pack in place with a content-addressed pack containing a representable new/newer witness. Fixed reserve count, free-slot headroom, and delete-then-create gaps are not correctness assumptions, and concurrent compactors cannot reset another scope or require Human repository surgery while pack capacity remains.
