...
- Status-only/fake manifests never become authority. A protected publication becomes committed only after the Authority App writes an OIDC-signed create-only witness binding the exact manifest ID, exact chunk IDs, second manifest proof, and current greatest authority body. Readers validate that witness directly instead of walking live reverse-chronological status pages, so hostile appends cannot move a seek/scan frontier during recovery. Greatest signed witnesses remain packed into immutable content-addressed Authority buckets. Hard-cap compaction atomically transforms an occupied verified source slot into its replacement pack, and can atomically transfer a redundant source/reserve slot to a required checkpoint; fixed reserve count and delete-then-create headroom are not correctness assumptions, and concurrent compactors cannot reset another scope or require Human repository surgery.
