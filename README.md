...
Fake manifests must not create unbounded read amplification, and an attacker with `issues:write`, `statuses:write`, or `contents:write` must not be able to erase or starve recovery progress. Locator comments and every `refs/fugue/**` custom Git ref are presentation/untrusted only. Recovery processes bounded status work under a frozen status-ID ceiling, then writes each OIDC-signed progress checkpoint through a dedicated Fugue Authority GitHub App into repository Actions Variables. The normal workflow `GITHUB_TOKEN` permission set has no Variables permission; the App credential is exposed only through the protected-base `fugue-authority` environment. Reverse-chronological GitHub API page numbers are not durable coordinates: each slice uses a bounded ID search to relocate the signed low-water `before_id`, so even thousands of newer statuses appended between slices cannot push unseen frozen records away faster than recovery advances. Readers validate checkpoints and select greatest progress. The checkpoint plane keeps one monotonic greatest cursor for every resumable d3 identity and never interprets “caught up to the current frozen ceiling” as retirement. Capacity is reduced by immutable, content-addressed bucket packs of the original signed cursor bodies; a replacement pack is created and re-read before any source containing its progress is removed. Reserved empty Authority variables provide normal hard-cap headroom; if interrupted writes consume all reserves, a required cursor allocation performs repository-wide safe compaction of redundant Fugue recovery sources before it may fail, preserving every scope's sole greatest cursor and every unrelated variable. Concurrent compactors therefore preserve each scope even when hostile statuses arrive after an earlier caught-up checkpoint.
