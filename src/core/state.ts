...
export async function publishDurableProtocolRecord(
  ...
  for (let attempt = 0; attempt < DURABLE_WRITE_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (!(await verifyProtocolPublicationBodyAtRevision(github, signedBody, input.publisherSha, input.publicationTimestamp))) {
...
async function validateDurableBody<T>(
  ...
  try {
    if (!(await verifyProtocolPublicationBodyAtRevision(github, body, options.publisherSha, timestamp))) return undefined;
  } catch {
    return undefined;
  }
  return value;
...
interface LocalRecoveryMutationSettlement {
  guardName: string;
...
const localRecoveryMutationSettlements = new WeakMap<FugueGitHub, LocalRecoveryMutationSettlement>();
const localRecoveryMutationGenerations = new WeakMap<FugueGitHub, number>();

function invalidateRecoveryReadSession(github: FugueGitHub): void {
  recoveryReadSessions.delete(github);
}

function localRecoveryMutationGeneration(github: FugueGitHub): number {
  return localRecoveryMutationGenerations.get(github) ?? 0;
}

function beginLocalRecoveryMutationSettlement(
  github: FugueGitHub,
  guardName: string,
): LocalRecoveryMutationSettlement | undefined {
  if (localRecoveryMutationSettlements.has(github)) return undefined;
  let resolve!: () => void;
  const settled = new Promise<void>((done) => { resolve = done; });
  const settlement = { guardName, settled, resolve };
  localRecoveryMutationSettlements.set(github, settlement);
  return settlement;
}

function settleLocalRecoveryMutation(github: FugueGitHub, guardName: string, rotated: boolean): void {
  const settlement = localRecoveryMutationSettlements.get(github);
  if (!settlement || settlement.guardName !== guardName) return;
  localRecoveryMutationSettlements.delete(github);
  if (rotated) {
    localRecoveryMutationGenerations.set(github, localRecoveryMutationGeneration(github) + 1);
  }
  settlement.resolve();
}

async function waitForLocalRecoveryMutation(github: FugueGitHub): Promise<boolean> {
  const settlement = localRecoveryMutationSettlements.get(github);
...
async function ensureRecoveryMutationGuardIdle(github: FugueGitHub): Promise<string | undefined> {
  ...
  if (!(await createFugueAuthorityVariable(github, RECOVERY_MUTATION_GUARD_IDLE, initial))) {
    ...
    const leaves = variables.filter((entry) => /^FUGUE_D3_[0-9A-F]{16}_[0-9A-F]{16}$/i.test(entry.name));
...
async function restoreRecoveryMutationGuardIdle(github: FugueGitHub, guardName: string, guardValue: string): Promise<void> {
  ...
  if (await replaceFugueAuthorityVariable(github, guardName, guardValue, RECOVERY_MUTATION_GUARD_IDLE, nextIdle)) return;
...
async function rollbackGuardedRecoveryMutation(
  ...
  if (guard.source_name && guard.source_value !== undefined) {
    ...
    if (source === undefined && target === guard.target_value) {
      if (!(await replaceFugueAuthorityVariable(github, guard.target_name, guard.target_value, guard.source_name, guard.source_value))) {
        throw new CanonicalWorkStateIntegrityError(
...
async function acquireRecoveryMutationGuard(
  ...
  const local = beginLocalRecoveryMutationSettlement(github, name);
  if (!local) return undefined;
  let acquired = false;
  try {
    acquired = await replaceFugueAuthorityVariable(github, RECOVERY_MUTATION_GUARD_IDLE, idle, name, value);
    return acquired ? { name, value } : undefined;
  } finally {
    if (!acquired) settleLocalRecoveryMutation(github, name, false);
  }
}
...
async function releaseRecoveryMutationGuard(github: FugueGitHub, token: { name: string; value: string }): Promise<void> {
  try {
    ...
    settleLocalRecoveryMutation(github, token.name, true);
  }
}
...
async function verifiedRecoveryEntriesForBucket(
  ...
  for (const variable of variables) {
    ...
    if (!cursor || recoveryCursorBucket(cursor) !== bucket) continue;
...
async function captureRecoveryReadSession(github: FugueGitHub): Promise<RecoveryReadSession> {
  ...
  for (let attempt = 0; attempt < RECOVERY_READ_BUILD_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (!epoch) {
        const waited = await waitForLocalRecoveryMutation(github);
        if (waited || localRecoveryMutationGeneration(github) !== localGeneration) {
          throw new RecoveryReadEpochChangedError(
            "A local protected recovery mutation completed while the bounded reader was starting.",
          );
        }
        throw new DurableProtocolRecoveryPendingError(
...
export async function publishCanonicalWorkState(
  ...
  if (logical) {
    if (current) {
      ...
      if (parsed.authority_sequence !== expectedSequence || parsed.parent_digest !== canonicalWorkStateDigest(current)) return false;
