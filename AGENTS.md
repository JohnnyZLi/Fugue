...
21. Durable status discovery is bounded, monotonic, and destruction-resistant under the Issues/Statuses/Contents writer threat. Readers never select authority from locator/receipt comments or custom Git refs. Recovery freezes a status-ID ceiling and persists each OIDC-signed low-water/materialization checkpoint as a write-once repository Actions variable created through a dedicated Fugue Authority GitHub App with repository Variables permission. Ordinary `GITHUB_TOKEN`, including candidate `contents:write`, has no Variables permission. Readers validate surviving checkpoint values and choose greatest progress. Recovery deterministically retains one greatest checkpoint per exact d3 scope and one lexicographic survivor for equal progress. Compaction never evicts another active scope's sole cursor; under finite repository capacity it may reclaim only quiescent completed cursors after same-scope duplicates are removed. Concurrent compactors therefore preserve monotonic progress even when more than 24 scopes are recovering at once, rather than forcing another active scope to restart.
