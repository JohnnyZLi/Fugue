...
21. Durable status discovery is bounded, monotonic, and destruction-resistant under the Issues/Statuses writer threat. Readers never select authority from locator/receipt comments. Recovery freezes a status-ID ceiling and persists its low-water/materialization state as an OIDC-signed same-tree Git commit behind a deterministic `refs/fugue/recovery/*` ref advanced only by fast-forward. Deleting issue comments or appending statuses cannot erase that checkpoint or reset page-one progress; fixed per-call manifest-proof/chunk budgets still bound work.
...
23. Each signed Integration request authorizes exactly one protected attempt-1 start with a fresh 256-bit one-use dispatch capability. Only its digest is durable in d3 and in an OIDC-signed dispatch-anchor Git ref. Before checkout/setup/build, the protected Integration workflow consumes that capability once and fast-forwards the ref to an OIDC-signed run-start commit carrying the exact run ID and attempt 1. This run-start ref—not filtered Actions search—is binding authority, so arbitrary same-request run flooding cannot replace the identity. Once the start boundary is durable, disappearance of the exact run without durable terminal evidence fails closed to terminal failure after the recovery grace period; only an actually observed cancellation/abortion is retryable. `workflow_run` completion is an optimization for prompt sealing, never the sole durable distinction between failure and transport retry.
...
src/core/state.ts              bounded d3 durable-record + signed recovery-ref/work/Coordinator authority
