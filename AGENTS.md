...
21. Durable status discovery is bounded and monotonic. Readers never select authority from locator/receipt comments. Recovery freezes a status-ID ceiling and advances a signed low-water/page cursor across fixed per-call manifest-proof and chunk budgets; newly appended statuses do not reset that ceiling or page-one progress. Authenticated manifest ranges are materialized independently of adjacent status placement.
22. Coordinator Human intent is preserved before canonicalization and ordered by immutable issue revision (`issue_updated_at`) plus event identity. Authorized issue runs use non-replacing event concurrency, publish the full immutable snapshot to d3 before work mutation, and refuse an older slower event once a newer revision is durable. Coordinator locator comments are repairable hints only.
23. Each signed Integration request binds to the globally earliest causally valid protected workflow-run ID at attempt 1 across all Actions pages. Protected non-replacing `workflow_run` completion events seal failure/cancellation even when the run object is deleted before prepare or reconciliation. Missing run objects stay fail-closed pending; only an observed cancelled/aborted first attempt permits a fresh request. Later same-request runs, reruns, replayed receipts, and run deletion cannot replace or erase a genuine terminal first-attempt result.
