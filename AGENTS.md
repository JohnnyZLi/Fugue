...
30. Protected Integration never treats repository-writable Deployment, Deployment Status, workflow-run presentation, actor/login, environment/ref/SHA matching, or the public request/token/title as attempt-binding authority. Immediately after the workflow-dispatch POST creates an attempt, a first protected `witness` job runs before the `fugue-authority` environment audit, Authority App token mint, checkout, build, or custom run-start publication and emits a content-bound OIDC-signed Integration run witness on the protected base commit. The signed body binds the exact request ID, PR, protected base SHA, one-use dispatch-secret digest, immutable Authority anchor name, exact `GITHUB_RUN_ID`, and run attempt 1; the commit comment carrying it is transport only, so copying a genuine witness can only replay the same signed run identity and mutating any bound field invalidates the proof. Lost-bind recovery chooses the globally lowest valid protected witness for that durable request, while legacy Deployment/Status reads are explicitly firewalled from run election. Once that witness, returned dispatch binding, or protected run-start is durable, shared `actions:write` deletion of the Actions run cannot erase attempt 1 and disappearance fails closed to terminal failure unless cancellation was actually observed. A request with no protected witness/binding/run-start after the recovery grace window remains never-sealed transport and may abort/retry; candidate Actions/Issues/Statuses/Deployments write authority cannot mint a matching secret-bound protected witness or forge PASS/failure/run identity.
...
src/core/integration-status.ts protected witness binding + one-use dispatch/run-start/terminal authority
src/core/integration-run-witness.ts exact protected attempt-1 OIDC run-identity witness
src/core/integration-status-legacy.ts preserved Integration state/recovery core behind deployment firewall
