...
21. Durable status discovery is bounded, monotonic, and destruction-resistant under the Issues/Statuses/Contents writer threat. Readers never select authority from locator/receipt comments or custom Git refs. Recovery freezes a status-ID ceiling and persists each OIDC-signed low-water/materialization checkpoint as a write-once repository Actions variable created through a dedicated Fugue Authority GitHub App with repository Variables permission. Ordinary `GITHUB_TOKEN`, including candidate `contents:write`, has no Variables permission. Readers validate surviving checkpoint values and choose greatest progress. Recovery keeps a bounded working set, deterministically retains one lexicographic survivor for equal progress, and compacts stale checkpoints before it needs a free repository-variable slot; concurrent cleanup therefore cannot cross-delete every greatest checkpoint and ordinary recovery cannot exhaust the finite Variables namespace.
...
23. Each signed Integration request authorizes exactly one protected attempt-1 start with a fresh 256-bit one-use dispatch capability. Only its digest is durable in d3. A dedicated Fugue Authority GitHub App, available only through the protected-base `fugue-authority` environment and carrying repository Variables permission rather than ordinary Actions/Contents authority, uses one bounded short-lived authority slot per active PR. The environment's restrictive protected-default-branch deployment policy is an external bootstrap invariant that must be configured and verified before the App private key is installed; an in-job policy audit is defense-in-depth drift detection, not the security boundary. Before checkout/setup/build, attempt 1 proves the capability and transitions that same authority slot once to an OIDC-signed run-start value carrying the exact run ID and attempt 1. As soon as that run binding (or an abort) is durably committed to d3, the transient slot is reclaimed; retries therefore do not accumulate Variables. Candidate `GITHUB_TOKEN` cannot pre-create, delete, redirect, rewind, or replace this authority; custom Git refs are never consulted. Once the start boundary is durable, disappearance of the exact run without durable terminal evidence fails closed to terminal failure after the recovery grace period; only an actually observed cancellation/abortion is retryable.
