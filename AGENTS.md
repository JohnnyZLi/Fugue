...
20. The d3 durable-record commit itself is protected authority, not merely signed prospective content carried by candidate-writable statuses. The final manifest carries a second protected OIDC proof binding the ordered exact server-assigned status ID of every data chunk, body digest, authority order, random bundle key, and 128-bit commit nonce. Hostile statuses interleaved after the last publisher check cannot make an accepted manifest point at an unreconstructible body, and candidate `statuses:write` cannot finish an aborted prospective publication or repackage committed content.
21. Durable status discovery is bounded, monotonic, and destruction-resistant under the Issues/Statuses/Contents writer threat. Readers never select authority from locator/receipt comments or custom Git refs. Recovery freezes a status-ID ceiling and persists each OIDC-signed low-water/materialization checkpoint as a write-once repository Actions variable created through a dedicated Fugue Authority GitHub App with repository Variables permission. Ordinary `GITHUB_TOKEN`, including candidate `contents:write`, has no Variables permission. Reverse-chronological GitHub API page numbers are never persisted or compared as progress: each bounded slice re-seeks its signed `before_id` by status ID before scanning the frozen range, so arbitrary newer appends between slices cannot shift unseen frozen authority away. Readers validate surviving checkpoint values and choose greatest progress. Recovery keeps one monotonic greatest cursor for every resumable identity and never treats a merely caught-up ceiling as retirement. Greatest signed cursors are compacted into immutable content-addressed bucket packs on the separate Authority-Variables plane; a replacement pack is durable before its source variables are deleted. Reserved empty Authority slots provide hard-cap compaction headroom, and if crashes consume every reserve, a required checkpoint allocation runs repository-wide safe recovery compaction before failing, deleting only redundant Fugue recovery sources while retaining every scope's sole greatest cursor and unrelated variables. Concurrent compactors can therefore converge without cross-deleting another scope or resetting it to page 1.
